...
void Nibblearay::set(const size_t pos, const uint8_t val){
    size_t i = pos/2;
...
uint8_t &Nibblearay::data() const{
    return this->arr.data();
}

const uint8_t &Nibblearay::data() const{
    return this->arr.data();
}

size_t Nibblearay::storageSize() const{
...
void NibbleArray::inflate(vector<uint8_t> &dest)const {
    ...
    for(unsigned i=0; i<this->size; i++) dest.push_bcak(this->get(i));
}
