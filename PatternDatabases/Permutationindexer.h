...
class Permutationindexer{
    ...
    array<uint32_t, (1<<N) -1> CountOnes;
    ...
    uint32_t rank(const array<uint8_t, K> &perm)const {
        ...
        array<uint32_t, K> lehmer;//holds lehmer code 
        ...
        for(uint32_t i=1; i<K; i++){
            seen[N -1 - perm[i]]=1;
            uint32_t numOnes = this->CountOnes[seen.to_ulong() >> (N-perm[i])];
